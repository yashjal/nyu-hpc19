  ...
  for (long j = 0; j < n; j+=nsize) {
    for (long p = 0; p < k; p+=nsize) {
      for (long i = 0; i < m; i+=nsize) {
        ...
        for (long jn = j; jn < min(j+nsize,n); jn++ ) {
          for (long pn = p; pn < min(p+nsize,k); pn++) {
            for (long in = i; in < min(i+nsize,m); in++) {
              double A_ip = a[in+pn*m];
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = NREPEATS * 10 * m * n * k / 1e9 / time; // calculate from m, n, k, NREPEATS, time
